---
layout: default
title: Privacy Policy
---

# TranslateKB Privacy Policy

**Effective:** 2026-05-17
**Contact:** amin.benarieb@gmail.com

TranslateKB does not collect, store, or transmit any personal data.

## What we don't do

- We don't log your keystrokes.
- We don't send your draft text to any server.
- We don't have user accounts, sign-in, or any backend.
- We don't include third-party analytics, advertising SDKs, or crash reporters.
- We don't share data with anyone.

## What stays on your device

- Your translation preferences — source language, target language, default tone — stored in shared `UserDefaults` (App Group).
- A monthly translation counter so the main app's Settings screen can show usage stats.
- Apple-managed Translation framework language packs, downloaded by iOS on demand.

None of this data leaves your iPhone.

## Network access

The keyboard extension requires "Allow Full Access" in iOS Settings. This permission grants the extension the network entitlement so Apple's on-device Translation framework can download language packs as needed. Those downloads go directly from your iPhone to Apple's CDN — TranslateKB does not see them, proxy them, or transmit any content elsewhere.

## Children

TranslateKB is not directed at children and does not knowingly collect data from anyone, including children under 13. There are no in-app purchases, ads, or tracking.

## Changes

Any future change to this policy will be published at this URL with an updated effective date.

## Questions

Contact: [amin.benarieb@gmail.com](mailto:amin.benarieb@gmail.com)
